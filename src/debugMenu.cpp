...
void DebugMenu::print(int fps)
{
    ...
    for(int i = 0; i < MAX_ENTITY; i++)
    {
        if (Entity::entityTab[i] != NULL)
        {
            ...
            SDL_RenderFillRect(render, &Entity::entityTab[i]->hitbox);

            /* print extra data */

            char coo_txt[64];   SDL_memset(coo_txt, 0, 64);
            snprintf(coo_txt, 64, "Coo (X,Y): %d %d", 
                                    Entity::entityTab[i]->position.x,
                                    Entity::entityTab[i]->position.y
                    );

            char atk_txt[64];   SDL_memset(atk_txt, 0, 64);
            snprintf(atk_txt, 64, "Atk : %d ( %d + %d)", 
                                    Entity::entityTab[i]->base_atk + Entity::entityTab[i]->item_atk_bonus(),
                                    Entity::entityTab[i]->base_atk,
                                    Entity::entityTab[i]->item_atk_bonus()
                    );

            char hp_txt[64];   SDL_memset(hp_txt, 0, 64);
            snprintf(hp_txt, 64, "Hp : %d ( %d + %d)", 
                                    Entity::entityTab[i]->base_hp + Entity::entityTab[i]->item_hp_bonus(),
                                    Entity::entityTab[i]->max_hp,
                                    Entity::entityTab[i]->item_hp_bonus()
                    );

            char speed_txt[64];   SDL_memset(speed_txt, 0, 64);
            snprintf(speed_txt, 64, "Speed : %d ( %d + %d)", 
                                    Entity::entityTab[i]->speed + Entity::entityTab[i]->item_speed_bonus(),
                                    Entity::entityTab[i]->speed,
                                    Entity::entityTab[i]->item_speed_bonus()
                    );

            this->name_texture  = write(render, &this->rect_name,  this->debugFont, Entity::entityTab[i]->name.c_str());
            ...
            this->rect_name.x  = Entity::entityTab[i]->hitbox.x;
            this->rect_coo.x   = Entity::entityTab[i]->hitbox.x;
            this->rect_hp.x    = Entity::entityTab[i]->hitbox.x;
            this->rect_atk.x   = Entity::entityTab[i]->hitbox.x;
            this->rect_speed.x = Entity::entityTab[i]->hitbox.x;

            this->rect_name.y  = Entity::entityTab[i]->hitbox.y - this->rect_speed.h - this->rect_atk.h - this->rect_hp.h - this->rect_coo.h - this->rect_name.h;
            this->rect_coo.y   = Entity::entityTab[i]->hitbox.y - this->rect_speed.h - this->rect_atk.h - this->rect_hp.h - this->rect_coo.h;
            this->rect_hp.y    = Entity::entityTab[i]->hitbox.y - this->rect_speed.h - this->rect_atk.h - this->rect_hp.h;
            this->rect_atk.y   = Entity::entityTab[i]->hitbox.y - this->rect_speed.h - this->rect_atk.h;
            this->rect_speed.y = Entity::entityTab[i]->hitbox.y - this->rect_speed.h;
